...
Scene_CharacterSelect.prototype.loadFiles					= function(){
	...
	this._maxPartySize = this.get_party_size();
};
...
Scene_CharacterSelect.prototype.get_party_size				= function(){
	return $gameVariables.value($gameSystem.sc_varIds().partySizeMax);
}
Scene_CharacterSelect.prototype.get_face_scale				= function(){
	return 0.5;
}
...
Scene_CharacterSelect.prototype.createFaces					= function(){
	 ...
	 for (var i = 0; i < this._faceBitmaps.length; i++) {
		  ...
		  this._facesSprites[i].scale.x = this.get_face_scale();
		  this._facesSprites[i].scale.y = this.get_face_scale();
...
Scene_CharacterSelect.prototype.createMembers				= function(){
	...
	for (var i = 0; i < this._maxPartySize; i++){
		...
		var _decal = [0,0];
		if(i > 3)_decal = [Moghunter.charSel_Face2S * 4, (Moghunter.charSel_Face2S+ 144)*-2];
		this._members[i].x = Moghunter.charSel_Face2X + _decal[0];
		this._members[i].y = Moghunter.charSel_Face2Y + (Moghunter.charSel_Face2S * i) + _decal[1];
...
Scene_CharacterSelect.prototype.updateMembers				= function(i){
	for (var i = 0; i < this._members.length; i++) {
		var _decal = 0;
		if(i > 3)_decal = 100;
		 if (this._members[i].x > Moghunter.charSel_Face2X + _decal) {
			 this._members[i].x -= 5;
			 this._members[i].opacity += 25;
			 if (this._members[i].x <= Moghunter.charSel_Face2X + _decal) {
				 this._members[i].x = Moghunter.charSel_Face2X + _decal;
...
Scene_CharacterSelect.prototype.refreshMembers				= function(i,actor_id){
	...
	var _decal = 0;
	if(i > 3)_decal = 100;
	this._members[i].x = Moghunter.charSel_Face2X + 50 + _decal;
...
Scene_CharacterSelect.prototype.isOnFace					= function(sprite) {
	 var cw = sprite.bitmap.width * this.get_face_scale() / 2;
	 var ch = sprite.bitmap.height * this.get_face_scale() / 2;
